...
def mesh(indSet, stMesh):
    with open(indSet['nazwa']+'.inp','r') as f:
        txt = f.read()
    ...
    with open(indSet['nazwa']+'.inp','w') as f:
        f.write(text)

def calcInput(zm):
    zm['obroty'] = str((zm['obroty'] *((2.0*np.pi)/60.))**2.)
...
# PhysicalSurfaceALL, CENTRIF,OBROTY,0.,0.,0.,0.,0.,1.
#==============================================================================
    t = ["GESTOSC","MYOUNG","POISS","G_WEW","G_ZEW","PODSTAWA","LOPATKI",
         "WLOT","ALL","OBROTY","NAZWA"]
    for i in t:
        new = i.lower()
        inpFile = inpFile.replace(i,str(zm[new]))
